...
def generate_pdf_from_questions(questions_json, subject, blooms_level, max_marks):
    pdf = QuestionPDF()
    pdf.add_page()
    ...
    # Subject (Left)
    pdf.cell(100, 8, f"Subject: {subject}", ln=0, align="L")

    # Max Marks (Right)
    pdf.set_x(140)
    pdf.cell(50, 8, f"Max Marks: {max_marks}", ln=1, align="R")
    ...
    for i, q in enumerate(questions_json):
        ...
        if i % 2 != 0:
            # Shift the text slightly and add "OR"
            display_text = f"OR  {question_text}"
            pdf.set_font("Helvetica", "B", 11)
        else:
            display_text = question_text
            pdf.set_font("Helvetica", "", 11)

        full_string = f"{prefix}{display_text}"
        clean_line = full_string.encode("latin-1", "ignore").decode("latin-1")
        ...
        if i % 2 != 0:
            # Large gap after the pair (Q1/Q2 is done)
            pdf.ln(12)
            # Draw a small subtle separator line between pairs
            curr_y = pdf.get_y()
            pdf.set_draw_color(200, 200, 200)
            pdf.line(40, curr_y, 170, curr_y)
            pdf.ln(8)
        else:
            # Small gap between Q1 and its "OR" counterpart Q2
            pdf.ln(2)
